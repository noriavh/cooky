...
import { Search, ChefHat, X, Sparkles, User, ShoppingBasket, Leaf } from 'lucide-react';
import { includesNormalized } from '@/lib/stringUtils';
import { motion } from 'framer-motion';
import { useRecipes, useMyRecipes, useOrigins, useTags } from '@/hooks/useRecipes';
...
import { useProfile, isDietCompatible, type DietType } from '@/hooks/useProfile';
...
const Inspiration = () => {
  ...
  const { data: myRecipes, isLoading: myRecipesLoading } = useMyRecipes();
  const { data: origins } = useOrigins();
  const { data: tags } = useTags();
  const { data: profile } = useProfile();
  const { data: products } = useShoppingProducts();
  const { data: favoriteIds = [] } = useRecipeFavorites();
  const { data: seasonalProducts } = useSeasonalProducts();
  
  const isLoading = recipesLoading || myRecipesLoading;
  ...
  // Get IDs of original recipes that have been copied by the user
  const copiedOriginalIds = useMemo(() => {
    if (!myRecipes) return new Set<string>();
    return new Set(
      myRecipes
        .filter(r => r.copied_from_id)
        .map(r => r.copied_from_id as string)
    );
  }, [myRecipes]);

  // Get IDs of my own recipes (to filter out copies from others)
  ...
  // Filter out:
  // 1. Original recipes that have been copied by the user
  // 2. Recipes from other cookiers that were copied from user's own recipes
  const visibleRecipes = useMemo(() => {
    if (!recipes) return [];
    return recipes.filter(recipe => {
      // Filter out originals that user has already copied
      if (copiedOriginalIds.has(recipe.id)) return false;
      
      // Filter out recipes from other cookiers that are copies of user's own recipes
      const isFromOtherUser = recipe.owner_id !== user?.id && 
        !(family?.id && recipe.family_id === family.id);
      if (isFromOtherUser && recipe.copied_from_id && myRecipeIds.has(recipe.copied_from_id)) {
        return false;
      }
      
      return true;
    });
  }, [recipes, copiedOriginalIds, myRecipeIds, user?.id, family?.id]);

  // Get unique owners from visible recipes
  const owners = useMemo(() => {
    if (!visibleRecipes) return [];
    const ownerMap = new Map<string, { id: string; name: string; isMe: boolean }>();
    
    visibleRecipes.forEach((recipe) => {
      if (!ownerMap.has(recipe.owner_id)) {
        const isMe = recipe.owner_id === user?.id || 
          (family?.id && recipe.family_id === family.id);
        ownerMap.set(recipe.owner_id, {
          id: recipe.owner_id,
          name: isMe ? 'Moi' : (recipe.owner?.username || 'Utilisateur'),
          isMe,
        });
      }
    });
    
    // Sort: "Moi" first, then alphabetically
    return Array.from(ownerMap.values()).sort((a, b) => {
      if (a.isMe) return -1;
      if (b.isMe) return 1;
      return a.name.localeCompare(b.name);
    });
  }, [visibleRecipes, user?.id, family?.id]);
  ...
  const filteredRecipes = useMemo(() => {
    ...
    const filtered = visibleRecipes.filter((recipe) => {
      ...
      if (filterOwner !== 'all') {
        ...
        } else if (recipe.owner_id !== filterOwner) {
          return false;
        }
      }
      ...
      // Filter by products (AND logic - recipe must have ALL selected products)
      if (filterProducts.length > 0) {
        const recipeProductIds = recipe.ingredients?.map(ing => ing.product_id).filter(Boolean) || [];
        const hasAllProducts = filterProducts.every(productId => recipeProductIds.includes(productId));
  ...
  }, [visibleRecipes, searchName, filterType, filterOrigin, filterDifficulty, filterDiet, filterTags, filterOwner, filterProducts, filterSeasonal, seasonalProductIds, seasonalProducts, user?.id, family?.id, favoriteIds]);

  const hasActiveFilters = searchName || filterType !== 'all' || filterOrigin !== 'all' || filterDifficulty !== 'all' || filterDiet !== 'all' || filterTags.length > 0 || filterOwner !== 'all' || filterProducts.length > 0 || filterSeasonal;
  ...
  const removeProduct = (productId: string) => {
    setFilterProducts(prev => prev.filter(id => id !== productId));
  };
      ...
      >
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl sm:text-2xl font-display font-bold">Inspiration</h1>
          </div>
          <ViewToggle viewMode={viewMode} onViewModeChange={setViewMode} />
        </div>
        
        <p className="text-sm text-muted-foreground">
          Découvrez toutes les recettes : les vôtres, celles de votre famille et celles de vos cookiers
        </p>
        ...
        {/* Filters Row 2 - Owner and Product filters */}
          ...
          <div className="flex flex-wrap gap-2 items-center">
            ...
            <Select value={filterOwner} onValueChange={setFilterOwner}>
              ...
              <SelectContent>
                ...
                {owners.map((owner) => (
                  <SelectItem key={owner.id} value={owner.isMe ? 'me' : owner.id}>
                    {owner.name}
                  </SelectItem>
                ))}
              </SelectContent>
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="outline" className="w-[200px] justify-between font-normal">
                  <span className={filterProducts.length === 0 ? 'text-muted-foreground' : ''}>
                    {filterProducts.length === 0 
                      ? 'Tous les produits' 
                      : `${filterProducts.length} produit${filterProducts.length > 1 ? 's' : ''}`}
                  </span>
                </Button>
              </PopoverTrigger>
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-1 max-h-64 overflow-y-auto">
                    {filteredProducts.map((product) => (
                      ...
                      >
                        <Checkbox
                          checked={filterProducts.includes(product.id)}
          ...
          <div className="flex flex-wrap gap-1.5">
            {filterProducts.map(productId => {
              ...
              return product ? (
                ...
                >
                  {product.aisles?.icon} {product.name}
                  <button 
        ...
        viewMode === 'grid' ? (
          ...
          >
            {filteredRecipes.map((recipe, index) => (
              ...
              >
                <RecipeCard recipe={recipe} showOwner showFavoriteButton isFavorite={favoriteIds.includes(recipe.id)} />
              </motion.div>
            ))}
          </motion.div>
        ) : (
          ...
          >
            {filteredRecipes.map((recipe, index) => (
              ...
              >
                <RecipeListItem recipe={recipe} showOwner showFavoriteButton isFavorite={favoriteIds.includes(recipe.id)} />
              </motion.div>
            ))}
          </motion.div>
        )
        ...
        <Card className="p-8 sm:p-12 text-center">
          ...
          {hasActiveFilters ? (
            ...
            <>
              ...
              <p className="text-muted-foreground mb-4 text-sm sm:text-base">
                Créez des recettes ou ajoutez des cookiers pour découvrir leurs recettes
              </p>
              <Link to="/recipes/new">
